...
Item {
    ...
    property var modes: converter ? converter.encodeFormats() : []
    property int modeIdx: 0
    property string lastResultText: ""
    property bool lastSuccess: true

    function targetExtForMode(mode) {
        if (mode === "Base64: encode") return ".b64"
        if (mode === "Base64: decode") return ".bin"
        if (mode === "Hex: encode")    return ".hex"
        if (mode === "Hex: decode")    return ".bin"
        return ".out"
    }
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            Text {
                text: "Base64 и Hex — encode / decode"
        ...
        RowLayout {
            ...
            AmberButton {
                ...
                onClicked: {
                    const ok = converter.convertEncode(
